...
workflow CARMACK_READPREP {
    ...
    // output-arm distribution, so that one payload is emitted with its meta as well as mixed in
    // below. prepare-reads writes a file per payload, so it has to be picked out by name.
    def ch_stats_json = CARMACK_PREPAREREADS.out.multiqc.map { meta, files ->
        def stats_json = files.find { mqc_file -> mqc_file.name.endsWith('.prepare_target_distribution_mqc.json') }
        if (!stats_json) {
            error("Sample '${meta.id}': prepare-reads wrote no *.prepare_target_distribution_mqc.json.")
        }
        [ meta, stats_json ]
    }
    ...
    stats_json    = ch_stats_json                    // channel: [ val(meta), path(prepare_target_distribution_mqc.json) ]
